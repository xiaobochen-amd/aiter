...
# Heads reduced per coarse CTA. The reduction is per (row, head) and shares
# nothing across heads, so this only changes packing: eight waves of one row
# read one 8 KB contiguous span per split.
_COMBINE_HEADS_PER_CTA = 1

# Longest split column the scalar LSE path takes. It rebuilds each split weight
# in VALU, removing two cross-lane butterflies, but spends one exp2 per split --
# so long columns keep the butterfly, whose cost is split-count independent.
...
def _compile_sparse_decode_direct_combine(ni: int, dv_slices: int):
    ...
    if dv_slices not in (1, 2, 4):
        raise ValueError("sparse decode combine takes 1, 2 or 4 Dv slices")

    # A slot is one reduction output fragment: one head's Dv cut into
    # `dv_slices` slices, each owned by one 64-lane CTA. The cut costs store
    # width and buys wave count, which is what decode row counts are short of.
    ...
    def kernel(
        ...
        # Row on grid-x, the row's slot group on grid-y, matching the
        # producer's split-major owner formula: gfx950 hands workgroup `i` to
        # XCD `i % 8`, so a row's readers land in the L2 domain that wrote it.

...
def _pick_inner_iter(seq: int, ng_total: int) -> int:
    ...
    while inner_iter < 4:
        ...
        current_groups = (ng_total + inner_iter - 1) // inner_iter
        candidate_groups = (ng_total + candidate - 1) // candidate
        if candidate_groups >= current_groups:
            break
        if seq * candidate_groups < min_producer_ctas:
            break

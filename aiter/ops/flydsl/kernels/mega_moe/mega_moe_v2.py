...
_DEFAULT_WORKSPACE_TOKENS_PER_RANK = 4096


class MegaMoEV2:
    """Fused distributed MoE with one reusable workspace per instance.

    Calls larger than the workspace require a rank-common collective token span.
    """
    ...
    def __init__(self, *, rank: int, world_size: int, model_dim: int, inter_dim: int, experts: int, topk: int,
        ...
        max_tok_per_rank: int, workspace_tokens_per_rank: int | None = None,
    ...
    # fmt: on
        ...
        workspace_tokens_per_rank = (
            min(self.mtpr, _DEFAULT_WORKSPACE_TOKENS_PER_RANK)
            if workspace_tokens_per_rank is None
            else int(workspace_tokens_per_rank)
        )
        if workspace_tokens_per_rank <= 0 or workspace_tokens_per_rank > self.mtpr:
            raise ValueError(
                "workspace_tokens_per_rank must be in "
                f"[1, max_tok_per_rank={self.mtpr}], got {workspace_tokens_per_rank}"
            )
        self.workspace_tokens_per_rank = workspace_tokens_per_rank
        self.swiglu_limit = float(swiglu_limit)
        if self.swiglu_limit < 0:
            raise ValueError("swiglu_limit must be non-negative")
        self.dev = torch.device("cuda", rank)
        self.max_recv = self.world_size * self.workspace_tokens_per_rank
        ...
        self.comb_cfg = FlyDSLDispatchCombineConfig(rank=self.rank, world_size=self.world_size,
            ...
            workspace_num_inp_token_per_rank=self.workspace_tokens_per_rank,
        ...
        self._chunked_out = None
        if self.workspace_tokens_per_rank < self.mtpr:
            self._chunked_out = torch.zeros(
                (self.mtpr, self.model_dim), dtype=torch.bfloat16, device=self.dev
            )
    ...
    def _allocate_dispatch_workspace(self, op, metadata_blocks):
        ...
        workspace = {
            ...
            "pair_order": torch.empty(self.workspace_tokens_per_rank * self.topk, dtype=torch.int32, device=self.dev),
    ...
    def _run_fused_stage1(self, x, wts, scales, topk_ids, stream=None, config: Stage1Config | None = None):
        ...
        if cur_tok > self.workspace_tokens_per_rank:
            raise ValueError(f"run_tokens={cur_tok} > workspace_tokens_per_rank={self.workspace_tokens_per_rank}")
        ...
        self._s1_mega(
            ...
            fuse_cap=self._s1_cap, fuse_mtpr=self.workspace_tokens_per_rank, fuse_scale_dim=self._s1_scale_dim,
    ...
    @staticmethod
    def _copy_chunk_output(dst, src, stream):
        if stream is None:
            dst.copy_(src)
        else:
    ...
    def _run_joint(
        ...
        stream,
        slice_output,
        collective_tokens_per_rank=None,
    ):
        if collective_tokens_per_rank is None:
            if run_tokens > self.workspace_tokens_per_rank:
                raise ValueError(
                    "collective_tokens_per_rank is required when run_tokens exceeds "
                    f"workspace_tokens_per_rank={self.workspace_tokens_per_rank}"
                )
            collective_tokens = run_tokens
        else:
            collective_tokens = int(collective_tokens_per_rank)
        if collective_tokens < run_tokens or collective_tokens > self.mtpr:
            raise ValueError(
                "collective_tokens_per_rank must cover local tokens and be no greater "
                f"than max_tok_per_rank={self.mtpr}, got {collective_tokens}"
            )
        if collective_tokens <= self.workspace_tokens_per_rank:
            config = self._select_config(collective_tokens)
            self._run_fused_stage1(
                x, wts, scales, topk_ids, stream=stream, config=config.stage1
            )
            stage2_slice = slice_output or self._chunked_out is not None
            out = self._run_stage2(run_tokens, stream, stage2_slice, config)
            if slice_output or self._chunked_out is None:
                return out
            self._copy_chunk_output(self._chunked_out[:run_tokens], out, stream)
            return self._chunked_out

        for start in range(0, collective_tokens, self.workspace_tokens_per_rank):
            schedule_end = min(
                start + self.workspace_tokens_per_rank, collective_tokens
            )
            local_end = min(schedule_end, run_tokens)
            local_start = min(start, local_end)
            chunk_tokens = local_end - local_start
            config = self._select_config(schedule_end - start)
            self._run_fused_stage1(
                x[local_start:local_end],
                wts[local_start:local_end],
                scales[local_start:local_end],
                topk_ids[local_start:local_end],
                stream=stream,
                config=config.stage1,
            )
            out = self._run_stage2(chunk_tokens, stream, True, config)
            if chunk_tokens:
                self._copy_chunk_output(
                    self._chunked_out[local_start:local_end], out, stream
                )
        return self._chunked_out[:run_tokens] if slice_output else self._chunked_out
        ...
        if out_tok is None:
            ...
            out_tok = (
                self.comb_op.shmem_comb_out_tok.view(torch.int8)[
                    : self.workspace_tokens_per_rank * cfg.combine_token_bytes
                ]
                .view(cfg.combine_dtype)
                .view(self.workspace_tokens_per_rank, cfg.combine_token_view_dim)
            )
    ...
    def _build_fused_stage2(self):
        ...
        for p2p_quant in ("none", "fp8_blockwise_1x32"):
            ...
            self._g2_invariants_by_quant[p2p_quant] = {
                ...
                "npes": int(comb_cfg.world_size), "max_tok": int(comb_cfg.workspace_num_inp_token_per_rank),
                "recv_cap": int(self.max_recv),
                "comb_inp_nbytes": int(comb_cfg.workspace_num_inp_token_per_rank) * int(k) * p2p_row_nbytes,

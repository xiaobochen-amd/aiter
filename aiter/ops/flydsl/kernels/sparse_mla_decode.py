...
# everywhere measured, and not from occupancy; the prologue is the one place a
# deeper burst pays -- see `_pick_xpf_prime`.
_XPF_DEPTH = 1
...
# CPol bits: SC0 in bit 0, SC1 in bit 4. Both set write and read past every
# cache, which is what a reducer on another L2 domain needs. Carrying the policy
# per instruction rather than fencing is what makes the fusion pay.
_CPOL_DEVICE = 0x11
...
def _arrive_and_wait(bar_ptr, arrivals: int, is_master):
    ...
    delta = is_master.select(fx.Int32(_BARRIER_SENSE - (arrivals - 1)), fx.Int32(1))
    old = llvm.atomicrmw(
...
def _split_weights(lse_vals, ni, scalar):
    ...
    if scalar:
        ...
        weights = [
            fx.Float32(fx.rocdl.exp2(T.f32, (v - max_lse).ir_value())) for v in lse_vals
        ]
        inv = fx.Float32(
    ...
    return [
        fx.Float32(fx.rocdl.readlane(T.f32, scale.ir_value(), fx.Int32(s).ir_value()))
...
def compile_sparse_mla_partial(
    ...
    def kernel(
        ...
        if fx.const_expr(True):
            ...
            # cover 128 contiguous bytes, so a request spans a full pair of
            # cache lines instead of 64 B.
            wide_slot = (
                fx.Int32(32) * (wave // fx.Int32(2))
            ...
            # stage needs anyway. Same wave writes and reads, so no barrier.
            key_base = lds.vlds.ptr + slot * fx.Int32(PITCH) + group * fx.Int32(16)
            out_record = (fx.Int64(tok) * n_groups + fx.Int64(split)) * H + fx.Int64(
                head
            )
            # The fused reducer sits on another CU, and sometimes another L2
            # domain, so these records are published past every cache. Both
            # sides carry the policy on the instruction; see `_CPOL_DEVICE`.
            ...
            def store_partial(vec, out_col):
                if fx.const_expr(fuse_combine):
                    ...
                    fx.ptr_store(vec, partial_ptr + out_record * DV + fx.Int64(out_col))
            ...
            def tile_of(k_i):
                ...
                # A ragged split leaves the last round short: those groups
                # re-read tile `split` and publish a negative index row, which
                # the padding mask scores as `-inf`, leaving the tile a no-op.
            ...
            def issue_index(k_i, fields="all"):
                ...
                # `fields` picks which of the three rows this call puts on
                # the wire, so the prologue can lift only the wide pair -- two
                # registers a tile instead of three.
                sl = {
                    "all": (slot,) + wide,
                    "wide": wide,
                    "narrow": (slot,),
            ...
            # Gathers in flight at once. Ordering below is load-bearing: the
            # Q loads issue first because vmcnt retires in order, so the
            # publishing stores drain only those, not the whole primed tile.
            ...
            rows_held = [issue_index(k, _ixf) for k in fx.range_constexpr(index_ahead)]
            ...
            # 576 = 2 x 256 + 64, so one wave publishes the remainder. Its
            # load stays here even though it forces the publish onto `vmcnt(0)`:
            # clamping out-of-range chunks in the other waves costs more.
            ...
            for k_i in fx.range_constexpr(inner_iter):
                # Keep the index stage `index_ahead` tiles ahead of the
                # gather it feeds, issued at the top of the body so those rows
                # retire behind the KV wait this body's LDS writes do anyway.
                if fx.const_expr(
                    index_ahead
                ...
                # Rescale P by `beta` before the fp8 pack rather than the PV
                # result after it: the factor rides in an exponent computed
                # anyway, and the fp8 grid then covers `beta * P`.
                ...
                # Fold the four per-wave sums here, once per tile. Deferring
                # to the last tile is exact but slower: these ds_read_b32 cover
                # the latency of the plds read that feeds PV.
                ...
                for j in fx.range_constexpr(8):
                    ...
                    # Seeding the MFMA's C operand with the rescaled
                    # accumulator makes the online-softmax update the same
                    # instruction as the product.
                    if fx.const_expr(seeded):
            ...
            if fx.const_expr(fuse_combine):
                # Publish, rendezvous, reduce. The stores are write-through,
                # so retiring them is all the release this needs; one thread per
                # CTA joins the rendezvous while the rest wait on the barrier.
                ...
                # A token's reducers are the CTAs that produced it: waves
                # number themselves `split * 4 + wave` and take slots round
                # robin, so no CTA is left holding the whole reduction.
                ...
                def round_reads(dv, slots, head_base):
                    ...
                    return (
                        ...
                        out_lane,
                        vals,
                        _lse_reads(lambda s: load_lse(s, rhead), n_groups, scalar_lse),
                        [
                ...
                # Every round's column goes on the wire before the first is
                # consumed, so round r+1's partials do not queue behind round
                # r's store -- one in-order vmcnt covers both.
                staged = [

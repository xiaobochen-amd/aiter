...
def _compile_sparse_mla_prefill():
    ...
    def kernel(
        ...
        # cover 128 contiguous bytes, so one request spans a full pair of cache
        # lines instead of 64 B.
        ...
        def issue_gather(block_start):
            ...
            for tile_index in range_constexpr(_QK_TILES_PER_WAVE):
                ...
                high_base = staged_row(block_start, tile_slot + wide_slot_col + 16) * (
                    _HEAD_DIM // 4
                )
        ...
        for key_start_iv, state in range(0, fx.Int32(_TOPK), _BLOCK_N, init=init):
            ...
            # The next block's rows are independent of everything below and
            # the published registers are free, so issue that gather now and let
            # QK, softmax and PV hide its latency.
            step = key_start + fx.Int32(_BLOCK_N)
            prefetched = issue_gather((step < fx.Int32(_TOPK)).select(step, key_start))
            ...
            for tile_index in range_constexpr(_QK_TILES_PER_WAVE):
                ...
                # copy PV needs anyway. Same wave, so no barrier.

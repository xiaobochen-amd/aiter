...
def dynamic_mxfp4_quant(
    ...
    # The ladder above is sized for prefill activations, where the N extent alone
    # already covers the part. A decode activation is only a few dozen rows tall,
    # so the same tiles leave the launch on a fraction of the CUs and the op runs
    # at ~2x its launch floor. Re-pick the tile when the grid underfills.
    num_sms = get_num_sms()
